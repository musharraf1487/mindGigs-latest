...
import { LayoutDashboard, Briefcase, Calendar, Clock, RefreshCw, Package, Rocket, DollarSign, Settings as SettingsIcon, LogOut, User } from 'lucide-react';
...
export function ExpertDashboard({ user, nav, logout, notify }) {
  ...
  const navItems = [
    ...
    { id: 'subscriptions', label: 'Subscriptions', icon: <RefreshCw size={18} color="var(--teal)" />, group: 'MANAGEMENT' },
    { id: 'products', label: 'Digital Products', icon: <Package size={18} color="var(--teal)" />, group: 'MANAGEMENT' },
  ...
  const Sidebar = () => (
    <div className="sidebar">
      ...
      <nav className="sidebar-nav">
        {['MAIN', 'MANAGEMENT', 'NETWORK', 'ACCOUNT'].map(group => (
          <div key={group}>
